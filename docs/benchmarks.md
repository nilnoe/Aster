...
| --- | --- | --- | --- | --- | --- | --- |
| 关闭最后一个窗口（BUG-018 ① 修复） | — | 无显著影响 | — | BUG-018 | 2026-08-03 | windowShouldClose 多一个 `frames.count` 分支（关闭频率极低，非热路径）；主线程看门狗 = 后台 1s 探针（心跳锁读写 O(1)），只在心跳停滞 ≥3s 时 NSLog，不触碰编辑 / 渲染 / 保存路径；Rust Core 零改动，基准不重跑 |
