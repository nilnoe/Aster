...
3. **退出流程**：未决登记（Session）全覆盖三分支——保存全部 / 全部不保存 / 取消。
4. **崩溃恢复**：经 Session 真实路径播种（开文档 → 编辑写缓冲 → 哨兵置非干净 →
   丢弃 session 模拟崩溃）→ 恢复 / 忽略决策，验证内容载回与缓冲行生命周期
   （ADR-013 v1.3 删除时机 2 / 保留规则 3）。**禁止假 id 播种**（42 / 5 / 9）——
   旧假 id 掩盖恢复 id 复用碰撞与残留状态（BUG-023，T-070 暴露）。
...
- `AppDelegate` 的退出提示与恢复提示经 `runModal()` 阻塞，测试无法直接驱动。
  ...
  T-070 起 `presentPendingDocsAlert` 按决策上下文区分：窗口关闭路径只问该窗口
  文档（closeDecisionDocId 非 nil，关 B 只问 B），退出路径问全部未决。
...
3. **状态机不变量测试（属性化）**：固定种子随机操作序列（打开 / 新建 / 编辑 /
   undo / redo / 保存 / 丢弃 / 崩溃恢复）驱动真实 AppDelegate，每步经 **Session
   公共 API** 断言守恒不变量（缓冲行集合 == 未决登记集合，ADR-013 v1.3；每个
   未决文档都有快照序号，BUG-011 泛化；序号唯一，BUG-010 泛化），终局断言
   保存全部成功且缓冲清空。种子固定保证 CI 确定性；多种子扩展状态空间。
   T-070 起不变量由 Core `Session` 方法保证（core/tests/session.rs 是 Core 层
   对应），App 层测试作为行为护栏 + 变异门禁捕获网。
...
## 关闭 / 退出决策契约（BUG-017，2026-08-03）

关闭按钮路径的 AppKit 生命周期契约：

1. 未决文档存在时关闭按钮 / ⌘W 必须经 `windowShouldClose` **先决策后关窗**
   （标准 macOS 模式）——保存全部成功 / 全部不保存放行，取消保持窗口打开。
   旧实现先关窗、在终止流程里弹提示，取消后无窗口导致终止反复重触发（死循环，
   独立 repro 实测）。
2. 决策抽为 `resolvePendingDocs()`，Cmd+Q（applicationShouldTerminate）与
   关闭按钮共用，两路径行为一致。
