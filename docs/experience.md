...
- **决策：** ADR-001 ~ ADR-025 全部 Accepted（索引见 `docs/adr/README.md`；
  ...
  ADR-024 / ADR-025）；宪法 V1.5（Rule 13~21：ADR 闭环 / 无消费者不交付 /
  文档机械门禁 / 性能数据驱动 / 域状态单一所有者 / 不变量由方法保证 /
  功能先冲突扫描 / 语义冻结 / 公共面总账机械化）。
...
- 宪法 V1.5：Rule 13~16（ADR 闭环 / 无消费者不交付 / 文档机械门禁 / 性能数据
  驱动）+ Rule 17~21（域状态单一所有者 / 不变量由方法保证 / 功能先冲突扫描 /
  语义冻结 / 公共面总账机械化）——新功能先问「属于哪个域、谁持有状态、
  不变量是什么」，域无单一所有者先收拢再实现（Rule 17）；保存域改动先确认
  方向（Rule 20）；改 bridge.rs 后跑 ffi-ledger.py（Rule 21）。
