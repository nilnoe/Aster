...
| BUG-018 | 新建 frame 关闭卡死（菊花旋转）+ 光标闪烁 Timer 保留环泄漏，三个机制：① **over-release（崩溃报告 2026-08-03 14:15，用户补充菊花=主线程卡死）**——新建 frame 的光标闪烁每 0.5s 触发一帧 Metal 绘制，关闭该 frame 时若窗口图层树拆毁期间仍有 in-flight drawable，CA 事务提交在 autorelease 释放 block 捕获对象时双重释放 → objc_release 坏指针崩溃（主线程栈：objc_release → _Block_release → NSConcretePointerArray dealloc → CA::Context::commit_transaction → app.run；只关新建 frame 崩、关最后窗口不崩与之吻合；测试复现不了 = 真实 CA 事务时序 + 定时器竞态）；修复 = `MetalView.beginClosing()`（windowShouldClose 放行时停定时器 + isClosing，`draw(in:)` 关闭态直接返回，拆毁期间零 in-flight drawable）；② **Design Bug（无窗口孤儿未决 × 终止重触发）**——关最后窗口只按该窗口文档决策，孤儿未决（⌘N / ⌘O 替换模型遗留、崩溃忽略登记）漏过 → 窗口先关 → 终止再弹提示 → 取消无窗口 → `applicationShouldTerminateAfterLastWindowClosed` 恒 true → 反复重触发终止 = 弹窗循环（BUG-017 同机制另一入口）；修复 = 最后窗口走全局决策（取消保窗 / 关窗前固化孤儿）；③ **保留环（已确认）**——blink Timer 以 target/selector 强持有 MetalView，且关闭后的窗口仍被 AppKit 保留；修复 = windowWillClose 确定性 stop + 看门狗自举用 block 弱引用。另加主线程看门狗（MainThreadWatchdog）诊断兜底 | ①Fixed（停笔 + 看门狗兜底）/ ②Fixed / ③Fixed | Implementation Bug（①）/ Design Bug（②）/ Implementation Bug（③） | — | BUG-018 切片（2026-08-03 收尾） | 2026-08-03 |
| BUG-019 | 关 frame B 弹 frame A 的未决提示：`windowShouldClose` 走全局 `resolvePendingDocs`，未决决策与窗口解耦——多 Frame 下关 B 会保存 / 丢弃 A 的文档（Frame × 关闭流冲突；BUG-018 卡死报告的疑似根因之一） | Fixed | Design Conflict | — | T-070 | 2026-08-03 |
| BUG-020 | 自动保存失败提示被跨文档吞掉：`bufferSaveErrorVisible` 是 App 全局单布尔——frame B 保存成功复位会永久吞掉 frame A 的失败提示（T-054 语义在多文档下失效） | Fixed | Design Conflict | — | T-070 | 2026-08-03 |
| BUG-021 | `makeFrame` 兜底 `?? 1`：存储故障时 `document_manager_open_scratch` 失败回退 id 1——两个 frame 抢同一 BufferId，未决 / 快照序号 / 固化基线三账本互相覆盖 | Fixed | Design Conflict | — | T-070 | 2026-08-03 |
| BUG-022 | DM 注册表随每次 ⌘N / ⌘⇧N / ⌘O 永久增长：`DocumentManager::close` 自 ADR-001 建成后零消费者，窗口关闭不回收文档（内存 + id 耗尽风险） | Fixed | Design Conflict | — | T-070 | 2026-08-03 |
| BUG-023 | 崩溃恢复 id 复用碰撞 + 残留状态：DM id 每进程从 1 重新分配，恢复新文档 id（常为 1）与崩溃遗留行 id 相同——旧实现「typeText 写缓冲后再删 latest」把刚写入的新行删掉，恢复后 ⌘S 报 scratch not found；真实路径播种（seedCrashedDocs）暴露，旧测试用 42 / 5 / 9 假 id 掩盖 | Fixed | Design Conflict | — | T-070 | 2026-08-03 |
| --- | --- | --- | --- | --- | --- | --- |
