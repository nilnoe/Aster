...
| BUG-018 | 新建 frame 关闭卡死（菊花旋转）+ 光标闪烁 Timer 保留环泄漏：① 用户补充症状「菊花旋转」= 主线程卡死——根因（Design Bug）：关闭**最后一个窗口**时 `windowShouldClose` 只按该窗口文档决策，无窗口的**孤儿未决文档**（⌘N / ⌘O 替换当前模型后遗留、崩溃忽略登记，T-046 多文档全程检查的既定状态）漏过：窗口先关 → 终止流程再弹提示 → 取消后应用无窗口，`applicationShouldTerminateAfterLastWindowClosed` 恒 true → AppKit 反复重触发终止 = 弹窗循环 / 卡死（BUG-017 同机制的另一入口；进程内穷尽复现此前正常，因 seam 绕过 runModal 且孤儿场景未覆盖，真实路径播种后由回归抓住）；修复 = 最后窗口走全局未决决策（取消保窗 / 关窗前固化孤儿）+ 主线程看门狗诊断（MainThreadWatchdog）；② 已确认缺陷：blink Timer 以 target/selector 强持有 MetalView 形成保留环，且实测关闭后的窗口仍被 AppKit 保留——修复 = windowWillClose 确定性 stop + 看门狗自举用 block 弱引用 | ①Fixed（机制修复 + 看门狗兜底；若用户场景仍复现，控制台 watchdog 行定位时间窗）/ ②Fixed | Design Bug | — | BUG-018 切片（2026-08-03 收尾：① 修复 + 2 回归 + 看门狗） | 2026-08-03 |
