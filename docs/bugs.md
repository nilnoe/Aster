...
| BUG-008 | 光标从 ASCII 行 ↓ 进入 CJK 行后落在字符内部（非 UTF-8 边界）：`move_cursor` 字节列目标未钳制到字符边界，后续 type_text / delete_backward 全部报 InvalidCharBoundary、按键静默丢失（违反 ADR-005 底线；属性测试差分排除 Up/Down，无覆盖） | Fixed | Implementation Bug | — | 本切片（T-035） | 2026-08-02 |
| --- | --- | --- | --- | --- | --- | --- |
