...
| --- | --- | --- | --- | --- | --- |
| T-049 CI-Bench 稳定性（ADR-021 v1.3） | 本切片 | 行为证据：本地实测 median + 200% 对比 exit 0；第二轮 CI 观测（+118.1% 误报、发布流水线同配置通过）数据驱动修订（Rule 16）；Rule 9 三问（脚本 1 函数 + 2 处阈值，0 抽象层 / 0 新依赖） | Pass | 无 | 2026-08-02 |
| T-048 发版 CI 检查修复 | 本切片 | 行为证据：本地实测 CI 同款 bench 命令 + 对比脚本（16 项 0 回归，exit 0）；CI-Rust 1.97 clippy 修复已在上一轮 CI 转绿；版本断言格式校验；Rule 9 三问（CI 配置 + 2 测试断言 + 1 脚本常量，0 抽象层 / 0 新依赖）；ADR-021 v1.2 修订留痕（阈值误报场景，Rule 16 数据驱动：实测噪声 +26%~+120%） | Pass | 无 | 2026-08-02 |
