...
| --- | --- | --- | --- | --- | --- |
| 数据结构评估落地（ADR-006 v1.1 + Phase 8 登记） | 本切片 | 行为证据：纯文档切片（ADR-006 追加评估进展节 4 项热点/缺口 + roadmap Phase 8 T-063~T-066 + benchmarks 缺口记录 + experience 速查）；评估基于实现逐结构复核与 T-023 基线引用（layout_build_1mb 0.31ms / store_scratch_save_10k 10.66ms / bench 只测末尾已核实）；Rule 9 三问（0 代码 / 0 抽象层 / 0 依赖）；Rule 16（未做任何结构替换，只登记基准缺口与门禁）；Rule 13（T-065 标注反转 ADR-023 需用户确认） | Pass | 无 | 2026-08-03 |
