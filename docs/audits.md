...
| --- | --- | --- | --- | --- | --- |
| T-043 崩溃恢复 v1（ADR-013 v1.1） | 本切片 | 行为证据：Core 2 + Bridge 1 + App 3 测试全绿（哨兵缺省/往返、缓冲枚举、恢复决策纯函数）；ADR-013 v1.1 修订留痕；Rule 9 三问（meta 表 1 行 + 3 方法 + 恢复提示，0 抽象层 / 0 新依赖）/ Rule 12（store.rs 288、AppDelegate 292 ≤300；core/src 2110、App 1945 在预算）/ Rule 14（3 FFI 消费者 = AppDelegate 恢复流程，同切片交付） | Pass | 无 | 2026-08-02 |
