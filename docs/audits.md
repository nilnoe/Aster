...
| --- | --- | --- | --- | --- | --- |
| T-050 App 集成测试套件（五组） | 本切片 | 行为证据：App 78 全绿（新增 15 项：启动链路 3 + 恢复 3 + 文档生命周期 4 + 退出三分支 4 + 端到端 keyDown 1，真实 NSApplication / NSWindow / 落盘断言）；Bridge 20 + Core 127 + fuzz 3000 全绿；swift-format / cargo fmt+clippy 零告警；Rule 3（AppDelegate 262 / Storage 167 ≤300；测试文件 49~144 ≤300）/ Rule 9 三问（2 个提示 seam 方法 + 移除 final，0 抽象层 / 0 新依赖；测试基建是永久资产）/ Rule 12（App 源码 2100 ≤5000，测试不计预算）/ Rule 14（无新公共 API，seam 为 App 模块内 internal）/ 踩坑：跨模块覆写须类体方法、RustString 错误消息须 toString、SQLite 跨连接可见性、@MainActor XCTest setUp | Pass | 无 | 2026-08-02 |
