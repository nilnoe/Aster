...
| --- | --- | --- | --- | --- | --- |
| BUG-010/011/012 修复（保存与恢复链路复审） | 本切片 | 行为证据：App 81 全绿（含 BugReproTests 3 个回归先红后绿：多文件保存互不覆盖 / 恢复后保存全部成功退出 / undo 回快照内容不置脏）+ Bridge 20 + Core 127 全绿；swift-format / cargo fmt+clippy 零告警；Rule 3（AppDelegate 274 / Storage 194 / BugReproTests 101 ≤300）/ Rule 9 三问（3 处保存状态修复 + 1 个比较基线字段，0 抽象层 / 0 新依赖，修复方案为最简：独立快照序号 / 写缓冲+登记 seq / 内容比较）/ Rule 12（App 源码 ≤5000）/ Rule 14（committedTextByDocId 为 App 模块内状态，非公共 API）；踩坑：History 相邻 Insert 合并规则再次干扰回归测试场景（改用选区替换拆步） | Pass | 无 | 2026-08-02 |
