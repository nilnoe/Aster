...
- [x] T-063 编辑热路径基准扩展（P0，ADR-006 v1.1 门禁前置）：1MB 文档中间
  insert / delete 10k、光标移动 1k、单次大 blob 缓冲 upsert——补齐中间编辑与
  移动成本数据，供 Gap Buffer / Rope / 维持 String 决策（现有 bench 只测末尾）
- [x] T-064 move_cursor 行索引缓存（P1）：Editor 持有 Layout 缓存（编辑失效、
  移动复用），移动 O(n) → O(log n)；基准对比（Rule 16）+ ADR-006 备注修订后实施
- [x] T-065 自动保存节流（P1）：连续按键合并缓冲写（~200ms 防抖），写频率降
  一个数量级；**反转 ADR-023「每次内容变更写入」粒度**（崩溃最多丢 ~200ms）——
  用户 2026-08-03 确认（性能优先方向，ADR-023 v1.8 冻结留痕）
- [x] T-066 SQLite WAL + synchronous 评估（P1）：缓解缓冲写放大（journal 写放大），
