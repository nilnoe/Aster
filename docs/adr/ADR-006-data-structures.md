...
| 字形缓存 / GPU 缓冲格式 | 按需字形图集（CoreText 栅格化进单张 RGBA8 纹理，按 font+glyph 去重）+ 每字形 6 顶点 quad 顶点流（32B/顶点，位置 + UV + 前景色）；图集满则整表重建 | ADR-016（T-012）落定：spike 规模下最简 GPU 文本路径；颜色走顶点便于 T-014 接 Theme |
