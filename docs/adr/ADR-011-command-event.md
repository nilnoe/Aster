...
- **T-013（编辑循环）** — context 增加文档访问（active buffer）原计划于本切片，现推迟到
  T-015（命令面板），见下方 T-013 修订备注。
...
- **T-013 修订**：原备注「文档 / 激活 Buffer 访问由 T-013 加入」推迟到 T-015（命令面板）：
  最小编辑切片不引入「激活文档」状态维度，键盘经 App → Bridge → Core 直连
  （ADR-017 决策）。
