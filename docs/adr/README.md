...
| ADR-013 | SQLite 存储层（Store）（v1.6：store_* FFI 由 ADR-025 session 承接；缓冲生命周期编排收拢；WAL + synchronous=NORMAL 落地，写放大 −96.5%，T-066） | Accepted | 2026-08-02 | 2 类型 + 10 方法 + 1 变体 | Core（store 模块）、app（恢复/保存流程） | [ADR-013-sqlite-store.md](ADR-013-sqlite-store.md) |
...
| ADR-023 | 保存语义：缓冲（SQLite）+ 快照（纯文本，v1.8：语义冻结期——保存域改动先确认方向再实现；自动保存 200ms 防抖反转粒度，T-065；FFI 面由 ADR-025 session 承接） | Accepted | 2026-08-02 | Snapshot 6 方法 | core（新增 snapshot、store、bridge）、app、docs | [ADR-023-disk-save.md](ADR-023-disk-save.md) |
...
| ADR-027 | 文档内容单一事实来源（v1.0：注册表与编辑会话共享同一 Buffer（Rc<RefCell<Buffer>>），结构上消除双副本失鲜；content_changed 删文本参数——Core 读活文，消除每键全量文本流；App 不再构造 Buffer；ADR-017 v1.1 / ADR-023 v1.8 修订留痕） | Accepted | 2026-08-03 | Session::editor + Editor::from_shared + 2 签名变更 + Bridge FFI +1 | core（editor、document_manager、session、bridge）、app、docs | [ADR-027-single-buffer.md](ADR-027-single-buffer.md) |
