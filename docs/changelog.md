...
- [docs/constitution.md](../docs/constitution.md) — Version 1.4 → 1.5：新增
  Rule 17~21——域状态单一所有者（Rule 17）/ 不变量由方法保证（Rule 18）/
  功能先冲突扫描（Rule 19）/ 语义冻结（Rule 20）/ 公共面总账机械化（Rule 21）。
  依据：T-070 治理纠偏总结——状态散落（BUG-009~023）、测试兜底（T-069 变异
  M3 存活）、新功能未扫冲突（T-069 × 关闭流 / T-054 × 多文档）、方向未定先写
  代码（ADR-023 一日六反转）、手抄计数漂移（ADR-013 头部 vs 索引）。WORKFLOW
  冲突扫描与 ADR-023 v1.7 冻结 / ADR-024 总账提升为宪法级；修订经项目所有者
  确认（用户「直接落实」）。
...
- docs(workflow)：WORKFLOW Analysis 新增强制**冲突扫描**清单——本切片触碰哪些
  既有子系统（保存状态机 / Frame / 关闭退出流 / IME / 渲染缓存 / 存储 / 编辑
  内核）、对每个子系统为何不破坏、哪些既有回归必须跑通；涉及保存 / 恢复 /
  关闭域必须先跑变异门禁再进入实现（T-070 治理纠偏；对应 T-069 × 关闭流、
  T-054 × 多文档、恢复 × id 契约三类既有冲突）。
- docs(adr)：ADR-023 v1.7——保存域**语义冻结期**：保存 / 恢复 / 关闭域任何
  语义改动必须先向项目所有者说明并确认方向再实现（T-037 → T-040 → T-042
  一日六反转前例；API 不稳定的根因 = 方向未定先写代码）。
- docs(roadmap)：新增 Phase 9 治理纠偏（T-070 收拢切片 + 冻结复核切片）；
  单关键路径收敛——Phase 7 剩余测试项并入对应功能切片前置，Phase 8 仅插队，
  不再平行三线。
...
### Fixed — 2026-08-03（T-059，BUG-016，已知限制契约固化）

- **BUG-016（Implementation Bug，忽略分支失管）**：崩溃恢复选「忽略」只登记
  最新缓冲文档——多缓冲文档场景下其余文档退出「保存全部」不覆盖、干净退出后
  内容困在缓冲（下次启动不再提示，除非再崩溃），违反 ADR-013 v1.4 保留规则
  3/4（不因忽略而失管 / 不留「没被问过」的文档）。
- fix(app)：忽略分支遍历**全部**缓冲文档登记未决 + 快照序号（已登记过的保留
  原序号，避免无谓换合并目标）。
- test：3 项契约测试固化已知限制（T-024 前 / T-029 前）——打开第二个文件只
  切换视图、前一个文档未决状态 / 缓冲行 / 快照序号保留且退出保存全部覆盖；
  恢复只呈现最新缓冲文档、其余保持可管；忽略后全部缓冲文档可保存。先红后绿
  （忽略用例旧实现报 42/43 未登记）。App 全量 99 项全绿（96 → +3）；Bridge 20
  + Core 127 不变；门禁零告警。

### Added — 2026-08-03（T-056，存储损坏与迁移契约）

- test(core)：4 项契约测试——损坏 buffer.sqlite（乱字节 / 合法头 + 截断体）
  打开返回 `Sqlite` 错误而非 panic（启动不崩底线）；旧 schema（user_version=0
  无 meta 表）打开后补齐表、迁移锚点推进到 v1 且可读写；只读目录
  `open_buffer` 失败干净（root 除外）；两个连接打开同一 buffer.sqlite 时
  已提交行跨连接可见（SQLite 文件锁串行化，App 单线程顺序操作）。
- test(app)：损坏 buffer.sqlite 启动不崩 + 失败可见（复用 T-054 提示路径）：
  存储未就绪、可继续编辑（未决保留）、保存失败提示准确。
- chore(core)：rusqlite 加入 dev-dependencies（同一已锁定 0.40.1，非新依赖，
  Rule 7/8 不触发）——仅测试构造 v0 数据库用。
