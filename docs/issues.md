...
| I-005 | P2 | 文档计数漂移与审计行未回填：experience 写 core 1676 行（实 1731）；audits 写 App 1483（实 1615）；T-032 审计行 commit 列仍为「本切片」（d867ac7） | Rule 15 机械门禁只覆盖 ADR 索引，未覆盖 experience / audits 计数 | T-036 | Open | — |
| I-006 | P2 | `.github/.DS_Store` 被提交进仓库 | 仓库卫生；macOS 用户目录元数据不应入库 | T-036 | Open | — |
